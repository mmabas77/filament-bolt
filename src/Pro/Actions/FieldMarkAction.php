...
class FieldMarkAction extends Action
{
    protected function setUp(): void
    {
        ...
        $this
            ->label(__('zeus-bolt::forms.fields.options.mark'))
            ->icon('tabler-bookmark')
            ->color('info')
            ->tooltip(__('zeus-bolt::forms.fields.options.mark_tooltip'))
            ->fillForm(fn (array $arguments, Repeater $component) => [
                'mark' => $component->getItemState($arguments['item'])['options']['mark'] ?? null,
            ])
            ->schema([
                Select::make('mark')
                    ->label(__('zeus-bolt::forms.fields.options.mark_label'))
                    ->options([
